...
def identify(img, draw=True):
    predictions = []
    results = plate_detector.predict(img)
    for result in results:
        ...
        for i, box in enumerate(boxes):
            ...
            if conf > opt.threshold:
                ...
                plate_image = img[y1:y2, x1:x2].copy()
                ...
                if draw:
                    ...
                    cv.putText(img, f'{current_class} {conf}', (x1, y2 + 50),
                ...
                if draw:
                    img = cv.putText(
                        img, f'{text_pred} {score}', (x1, y1 - 10), cv.FONT_HERSHEY_PLAIN, 3, (0, 255, 0), 3)
...
while True:
    ...
    frame, predictions = identify(frame, draw=True)
